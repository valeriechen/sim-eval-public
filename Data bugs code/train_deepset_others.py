...
for i in range(0,n,10):
  ...
  elif parser.dataset in ['bank', 'occupancy', 'shopping']:
    
    if parser.dataset == 'bank':
      X_train,y_train, X_test, y_test, X_t_train = load_bank_dataset(has_bug,parser.bug_type)
    
    if parser.dataset == 'occupancy':
      X_train,y_train, X_test, y_test, X_t_train = load_occupancy_dataset(has_bug,parser.bug_type)
    
    if parser.dataset == 'shopping':
      X_train,y_train, X_test, y_test, X_t_train = load_shopper_dataset(has_bug,parser.bug_type)
...
np.save(parser.dataset+"_dataset/dataset_"+parser.exp_type+"_"+str(n)+"_"+parser.bug_type+"_"+str(parser.set_size)+".npy", input_space)
np.save(parser.dataset+"_dataset/bugs_"+parser.exp_type+"_"+str(n)+"_"+parser.bug_type+"_"+str(parser.set_size)+".npy", has_bugs)
