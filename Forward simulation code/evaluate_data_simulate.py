...
class DeepSet(nn.Module):
    ...
    def __init__(self, set_size, in_features, set_features=200): #50
        super(DeepSet, self).__init__()
        self.in_features = in_features
        self.out_features = set_features
        self.feature_extractor = nn.Sequential(
            nn.Linear(in_features, 500), #100
            nn.ELU(inplace=True),
            nn.Linear(500, 200), #100
            nn.ELU(inplace=True),
            nn.Linear(200, set_features)
        )
...
parser.add_argument('-a', '--batch-size', metavar='B', type=int, nargs='?', default=64,
...
X = np.load("all_datasets/"+parser.dataset+"_dataset/dataset_"+parser.exp_type+"_2000_"+parser.bug_type+ "_" + str(parser.set_size) +".npy")
y = np.load("all_datasets/"+parser.dataset+"_dataset/bugs_"+parser.exp_type+"_2000_"+parser.bug_type+"_" + str(parser.set_size) +".npy")
...
X_test = np.load("all_datasets/"+parser.dataset+"_dataset/datasetval_"+parser.exp_type+"_250_"+parser.bug_type+ "_" + str(parser.set_size) +".npy")
y_test = np.load("all_datasets/"+parser.dataset+"_dataset/bugsval_"+parser.exp_type+"_250_"+parser.bug_type+"_" + str(parser.set_size) +".npy")
...
optimizer = optim.Adam(model.parameters(), lr=1e-2, eps=1e-3)
